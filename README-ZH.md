...
在打开一个新页面时，系统会渲染整个页面结合动画完成页面切换。这时，如果页面过于复杂，同样会出现页面切换的卡顿。借助分帧组件，将页面的构建逐帧拆解，通过 DevTools 中的性能工具查看。切换时一帧的峰值由 **112.5ms 降低到 30.2 ms**。
...
此外，也可以给 item 嵌套透明度/位移等动画，优化视觉上的效果（注意：这会带来额外的开销）
...
1、额外的构建开销：整个构建过程的构建消耗由「n * widget消耗 」变成了「n *（ widget + 占位）消耗 + 系统调度 n 帧消耗」。可以看出，额外的开销主要由占位的复杂度决定。如果占位只是简单的 Container，测试后发现整体构建耗时大概提升在 15 % 左右。

2、视觉上的变化：如同上面的演示，组件会将 item 分帧渲染，页面在视觉上出现占位变成实际 widget 的过程。但其实由于列表存在缓存区域，这在高端机或正常滑动情况下用户并无感知。而在低端机上快速滑动能明显切换的过程，但整体相比严重顿挫体感要好。
